...
# Parse DOI and download paper PDFs
def downloadPSC(doi_path='./Data/PolymerSolarCell/DOI_list.txt', output_path='./Data/PolymerSolarCell/PDFs'):
    dois = []
    downloaded_papers = os.listdir(output_path)
    with open(doi_path, 'r') as file:
        lines = file.readlines()
        for line in lines:
            dois.append(line.strip())
    for doi in dois:
        if doi.split('/')[-1] + '.pdf' not in downloaded_papers:
            paper_download_path = os.path.join(output_path, doi.split('/')[-1])
            downloadUrl = tools.GetDownloadUrl(doi)
            time.sleep(1)
            tools.downloadFileWithUrl(downloadUrl=downloadUrl, output_path=paper_download_path)
    return dois
...
def queryPubChem(smiles):
        cids = pubChem_REST_Smile(smile=smiles)
        synonyms = []
        for cid in cids:
            synonym = pubChem_REST_id2synonyms(cid)
            synonyms.extend(synonym)
        return synonym

# def image2Smiles(self, image_path):
#     smiles = predict_SMILES(image_path=image_path)
#     return smiles 

def obtain_Yinghao_doi(dataset_path, output_file):
    yinghao_papers = os.listdir(dataset_path)
    doi_list = []
    for yinghao_paper in yinghao_papers:
        with open(os.path.join(dataset_path, yinghao_paper), 'r') as yinghao_file:
            line = yinghao_file.readline()
        doi = json.loads(line)['text'].split()[1]
        doi_list.append(doi)
        
    with open(output_file, 'w+') as output_f:
        output_f.write('\n'.join(doi_list))

def obtain_Pranav_doi(dataset_path, output_file):
    ...
    for paper in papers:
        if counter > 100:
            break
        with open(os.path.join(dataset_path, paper), 'r') as paper_file:
            doi = json.load(paper_file)["DOI"]
            doi_list.append(doi)
        counter += 1
